...
\begin{equation*}
	...
	\end{array}
\end{equation*}
Here above, $\vp = (p,q)$ in some cases. A general reference 
...
\begin{example}[Strong polynomial tractability]
...
\begin{example}[Polynomial tractability]
Let $ T :(0,\infty) \times \mathbb{N} \times [0,\infty)^s \rightarrow (0,\infty)$
be defined by
\[
...
\begin{gather*}
	h^{-1}(\comp(\varepsilon,d))\le h^{-1}(\max(1,\comp(\varepsilon,d))) \le cT(\varepsilon^{-1},d)  \qquad \forall \varepsilon^{-1} + d \ge V_c, \\
...
\begin{example}[Weak Tractability]
	Suppose that 
	\[
	T(\varepsilon^{-1},d,\vp) = \varepsilon^{-s} + d^t, \quad \forall \varepsilon > 0, \ d \in \naturals, \quad \text{where } \vp = (s,t), \qquad h(x) = \exp(x) \quad \forall x \ge 0.
	\]
	Then Theorem \ref{thm:subhT} implies that 
	\[\lim_{\varepsilon^{-1}+d\rightarrow \infty} \frac{\log( \max(1,\comp(\varepsilon,d)))}{\varepsilon^{-s}+d^t} = 0 
	\iff 
	 \sup_{d\in\naturals}  \, \sum_{i=1}^\infty \exp(-c\, (\lambda_{i,d}^{-s} + d^t )) <\infty \qquad \forall c > 0.
	\]
