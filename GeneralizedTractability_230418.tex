...
%Note that the tractability function is bounded below away from zero even as $\varepsilon \to \infty$ \peter{[PETER: Is it?]}, so that the information complexity is not expected to vanish under arbitrarily loose error requirements.  
...
\]
%\peter{[PETER: Maybe using $\widetilde{\vp}$ instead of $\vp^\dagger$ in the definition of $\cp_{\textup{sopt}}$ is more intuitive?]}
\end{theorem}
...
\begin{equation} \label{eq:h_cond}
	...
	h(0)=1, \qquad  [h(x)]^c \le h(c x) \quad \forall x \ge 0, c > 0.
\end{equation}
\peter{Let us make the slightly stronger assumption on $h$ that
\[
h(x)\cdot h(y) \le h(x+y)\quad \forall x,y\in [0,\infty),
\]
and let us denote the inverse function of $h$ by $h^{-1}$. 
...
\fred{There is a problem here because I was assuming that 
\[
\lim_{x \to \infty} \frac{f(x)}{g(x)} = 0 \iff 
\lim_{x \to \infty} \frac{\log(f(x))}{\log(g(x))} = 0.
\]
But this is NOT the case, e.g., $f(x) = x^2$ and $g(x) = x^4$.  So we may need 
\begin{definition} \label{def:subhT}
	A problem is sub-$h \circ T$ tractable if
	\begin{equation*}
		\lim_{\varepsilon^{-1} + d \to \infty} \frac{h^{-1}(\comp(\varepsilon,d))}{T(\varepsilon^{-1},d,\vp)} = 0.
	\end{equation*}
\end{definition}
And NOT what I had below.}


\begin{definition} \label{def:subhT}
	A problem is sub-$h \circ T$ tractable if
	\begin{equation*}
		\lim_{\varepsilon^{-1} + d \to \infty} \frac{\comp(\varepsilon,d)}{h(T(\varepsilon^{-1},d,\vp))} = 0.
	\end{equation*}
\end{definition}
