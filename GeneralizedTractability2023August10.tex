...
\]
This means that, essentially, in the tractability analysis below, all criteria would be normalized by the first \peter{singular value} $\lambda_{1,d}$. For the sake of brevity and clarity of notation, we restrict ourselves to the absolute error criterion here, but presumably analogous results hold for the normalized setting.
...
\begin{multline}
  \exists \,C_{\vp} > 0 \text{ with }
    ...
    \iff   \exists L_{\vp} \peter{>0} \text{ with }
    \sup_{d \in \naturals}
     ...
     \theUB(d) := \begin{cases}
        ...
        \end{cases}
\end{multline}
The differences in these theorems are in the choices of $\thed$ and $\Omega$. Although some of these theorems are special cases of others, we prove each separately to allow the reader to become familiar with the arguments used as additional layers of complexity are added.
\Cref{thm:subhT} has a somewhat different form than above.  


In the following, we will denote the set of positive integers by $\naturals$ and the set of non-negative integers by $\naturals_0$. We write $\vzero$ to denote a vector with all components equal to zero, and $\vinfty$ to denote a vector with all components equal to $\infty$.
...
\end{equation}
which implies that the problem is expected to be no easier by decreasing the tolerance, $\varepsilon$, or increasing the dimension, $d$\footnote{This is why we make $T$ a function of $\varepsilon^{-1}$ rather than of $\varepsilon$.} Furthermore, increasing $\vp$ allows for a possibly looser bound on the information complexity.  Since there are an infinite number of positive \peter{singular values}, it also makes sense to assume that
...
\end{multline}
We note that the left side of the implication implies $n \ge N$ automatically since $n$ is an integer \peter{and} $n - N +1$ is assumed to be positive.
...
\]
Since the sequence of \peter{singular values} $\lambda_{1,d}, \lambda_{2,d}, \ldots $ is non-increasing, we have
...
\[
\sup_{d\in\naturals}\,\, \sum_{i=\lceil \kachi{L_{\vp'}} T(0,d,\vp')\rceil}^\infty \frac{1}{T(\lambda_{i,d}^{-1},d, \vp')}
\le
\sup_{d\in\naturals} \, \, \sum_{i=i (\infty,d,\vp')+1}^\infty \frac{1}{T (\lambda_{i,d}^{-1},d,\vp')} <
\infty.
\]
...
Next we show that if $\vp^* \in \widetilde{\cp}_{\textup{trct}}$, then $\vp^* \in \cp_{\textup{trct}}$.  If \kachi{$\vp^* \in \widetilde{\cp}_{\textup{trct}}$}, then \eqref{eq:tractiff4}  holds for all $\vp \in (\vp^*,\vinfty)$.  By the argument to prove the sufficient condition above, it follows that \eqref{eq:tract_def} must also hold for all $\vp \in (\vp^*,\vinfty)$. Thus, $\vp^* \in \cp_{\textup{trct}}$.
...
In this section, we present six examples relating to Theorem \ref{thm_main_strong_tract2} and Theorem \ref{thm_main_tract2}, both of which were \peter{shown} in Section \ref{sec:spt}, for the various notions of tractability that are listed in Table \ref{tab:commonT}. In \peter{the} subsequent sections, we will continue to present various examples.
...
\[
 \sup_{d\in\naturals}\, d^{-q} \sum_{i=\lceil L_{(p,q)} d^{q}\rceil}^\infty \min\{1,\lambda_{i,d}^p\}  < \infty \text{ for some } p,q \ge 0, L_{(p,q)} \peter{>0}.
\]
...
Having defined the tractability function \peter{as above}, we will now examine a specific case where we always set $d=1$,i.e.,
\[
...
%\peter{[But this is not what we discussed on Wednesday. The idea is that we use the same tractability function as for algebraic polynomial tractability, but consider the special case where we always set $d=1$,i.e., we use 
%\[
 %T(\varepsilon^{-1},1,\vp)= \max\{1,\varepsilon^{-p}\}\\qquad \forall \varepsilon > 0, \,  d \in \naturals, \, \vp \in [0,\infty)^2,\]
%as an upper bound on the information complexity. Then everything would correspond to our definitions of $T$-tractability and strong $T$-tractability. Kachi, can you please make this change here and also in Example 2? So, please make it clear that the tractability function for strong polynomial tractability is a specialization of the tractability function for polynomial tractability.]
%}
...
\end{example}
Next, we consider the exponential case where we replace $\varepsilon^{-1}$ by $\log(1+\varepsilon^{-1})$ and consider the same notions of tractability as for the algebraic case.
...
\begin{equation}\label{eq:exp_pt}
 \sup_{d\in\naturals}\, d^{-q} \sum_{i=\lceil L_{(p,q)} d^{q}\rceil}^\infty \frac{1}{[\max\{1,\log(1+\lambda_{i,d}^{-1})\}]^p} < \infty  \text{ for some } p,q \ge 0, L_{(p,q)} \peter{>0}.
\end{equation}
...
\]
Again, we will look at \peter{the} special case in which we always set $d=1$. In this case, we will define $T$ as follows.
\[
...
\end{example}
%\peter{TODO: Include a statement on optimality like in Ex. 2.}
...
\peter{Firstly, we remark that the role of $L_p$ in  \eqref{eq:exp_spt} corresponds to that of $C$ in \eqref{eq:KW_exp_spt}.
...
Secondly, it should be noted that our condition \eqref{eq:exp_spt} is easier to check in practice than condition \eqref{eq:KW_exp_spt}. Since in \eqref{eq:KW_exp_spt} the summation indices $i$ show up in the exponents of the summands, we need the exact ordering of the \peter{singular values} $\lambda_{i,d}$, whereas in \eqref{eq:exp_spt} we only have a summation of the values of $1/T(\lambda_{i,d}^{-1},d,\vp)$, and the exact order of the \peter{singular values} can be neglected. A similar
observation holds for \eqref{eq:exp_pt} and \eqref{eq:KW_exp_pt}.

\peter{The following proposition states that the conditions \eqref{eq:exp_spt} and \eqref{eq:KW_exp_spt}, as well as the conditions \eqref{eq:exp_pt} and \eqref{eq:KW_exp_pt} are indeed equivalent. 
The proof of this result is deferred to the appendix, where we will prove this statement for the latter two conditions, i.e., for the conditions on exponential polynomial tractability. The case of exponential strong polynomial tractability is not explicitly included, as it is actually simpler and can be treated analogously.
...
In the following example, we provide a brief summary of the above examples of tractability. We would like to emphasize that this example was previously known (see, e.g., \cite{GW06}) where in this case the roles of $\varepsilon^{-1}$ and $d$ are separated. In order to accomplish this, we will first define two functions $\phi$ and $\psi$ with positive values that are increasing in both of their arguments which will allow to take into account a variety of possible exponents of $\varepsilon^{-1}$ and $d$ \peter{[Is this really what we would like to express in this context?]}. The equivalent conditions for this example take the same form as those in Theorems \ref{thm_main_strong_tract2} and \ref{thm_main_tract2}.
...
We say that a problem is \peter{$T$-separably} tractable with parameter $\vp$ iff there exists a positive constant $C_{\vp}$ which does not depend on $\varepsilon$ and $d$ such that
\[
\comp(\varepsilon,d) \leq  C_{\vp} \,  \phi(\varepsilon^{-1},p)\, \psi(d,q)\qquad \forall \varepsilon >0, \, d\in\naturals.
\]
A problem is strongly \peter{$T$-separably} tractable with parameter $\vp$ iff the information complexity is independent of $d$, that is, there exists a positive constant $C_{\vp}$ which again does not depend on $\varepsilon$ and $d$ such that
\[
...
\]
since there are an infinite number of positive \peter{singular values}. Furthermore, we have that $\phi(\cdot,p)$ is non-decreasing, hence we define the following limit
\[
...
\end{example}
\kachi{Next, we introduce another notion of tractability called \emph{non-separable tractability}. See \cite{GW08}, which is probably where the term ``non-separable tractability" was first introduced.
In the following, we will define two functions $\hat{\phi}$ and $\hat{\psi}$ that take the roles of $\varepsilon^{-1}$ and $d$. As will be seen below, this kind of tractability function \peter{includes the special case} of the so-called \emph{quasi-polynomial tractability}.}
\
...
We say that a problem is \peter{non-separably} tractable with scalar parameter $p$ iff there exists a positive $C_{p}$ such that
...
\end{example}
For $\hat{\phi}$ and $\hat{\psi}$ of special form, the condition above can be simplified further as in the following two \peter{special cases} of non-separable tractability.
...
%\peter{[We should think about whether it makes sense to introduce Def. 3 and Example 7? Maybe just one of them is enough?]}
...
\[
 T(\varepsilon^{-1},d,\peter{p})= \exp [p(1+\log(\max\{1,\log(1+\varepsilon^{-1})\}))(1+\log (d))]  \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, \peter{p} \in [0,\infty).
\] This can be rewritten as
\[
T(\varepsilon^{-1},d,\peter{p}) = e^p(\max\{1,\log(1+\varepsilon^{-1})\})^{p(1+\log d)}d^p.
\]
...
\begin{theorem}\label{thm:subhT}
	Let $T$ be a tractability function as specified in \eqref{eq:Tspec} and satisfying \eqref{eq:Tconditions}.  \peter{Moreover, assume that 
	...
	\]
\end{theorem}
%\peter{[I am thinking that we need to assume slightly more in the hypothesis of the theorem, namely 
%$$
%\lim_{\varepsilon^{-1} + d \to \infty} T(\varepsilon^{-1},d,\vp) = \infty. 
%$$
%Why do I think so? Because it is not always true that $\lim_{\varepsilon \to 0} T(\varepsilon^{-1},d) = \infty$ for all $d \in \naturals$, and  $\lim_{d \to \infty} T(\varepsilon^{-1},d) = \infty$ for all $\varepsilon>0$ implly $\lim_{\varepsilon^{-1} + d \to \infty} T(\varepsilon^{-1},d) = \infty$. As a counter-example, let 
%$$
%T(\varepsilon^{-1},d)=(d+\varepsilon^{-1})/\sqrt{d \varepsilon^{-1}}.
%$$
%Then $\lim_{\varepsilon \to 0} T(\varepsilon^{-1},d) = \infty$ for all $d \in \naturals$, and  $\lim_{d \to \infty} T(\varepsilon^{-1},d) = \infty$ for all $\varepsilon>0$ is fulfilled. However, if you choose $\varepsilon=1/d$ for all $d$, then for this particular $\varepsilon$ you always have $T(\varepsilon^{-1},d)=2$.]
%}
...
As $S_c \ge 1/h(cT(\lambda_{1,1}^{-1},1))$ by \eqref{eq:Sclb}, it follows that $S_c \, h(cT(0,1)) \, h(cT(\varepsilon^{-1},d))  \ge 1$.  Define $\peter{\widetilde{S_c}} : = 2 S_c \, h(cT(\lambda_{1,1}^{-1},1))$, and note that $\widetilde{S}_c$ must also be finite for all $c > 0$.  It follows that
\begin{equation*}
...
\peter{By the hypothesis of this theorem, $\lim_{\varepsilon^{-1} + d \to \infty} T(\varepsilon^{-1},d) = \infty$.}  Therefore
\begin{equation*}
...
+ \sum_{i=1}^\infty \frac1{\peter{i}^2}
\right]
...
\peter{\quad [\mbox{why not write $\forall (\varepsilon^{-1}, d) \in \Omega$?}]}
\end{equation}
...
Note furthermore that we always have
\[
i(\varepsilon,\vp)\le C_{\vp}\, T(\varepsilon^{-1},1,\vp)+1 \le C_{\vp} T(\lambda_{i(\varepsilon,\vp),d}^{-1},1,\vp)+1 \qquad  \forall \varepsilon > \theM(d), \ d \in \mathbb{N},\peter{\quad [\mbox{why not write $\forall (\varepsilon^{-1}, d) \in \Omega$?}]}
\]
since
$T(\cdot,1, \vp)$ is non-decreasing.
...
\end{equation}
If \eqref{eq:rest_tractiff} holds for some $\vp$, let $\peter{\widetilde{\cp}_{\textup{rtrct}}} : = \{\vp^* : \peter{\eqref{eq:rest_tractiff}} \text{ holds }\forall \vp \in (\vp^*,\boldsymbol{\infty}) \}$.
...
\le
\sup_{d\in\naturals} \sum_{i \in \mathscr{I}_{d,\vp} \setminus\{1\} } \frac{1}{T (\lambda_{i,d}^{-1},\peter{d}, \tau \vp)} \\
&  \le  K_{\vp,\tau}\, C_{\vp}^\tau
\sum_{i=2}^\infty \frac{1}{(i-1)^\tau}
 \le K_{\vp,\tau}\, C_{\vp}^\tau
\zeta (\tau)
 < \infty \qquad \text{by \peter{\eqref{eq:rest_ineq}},}
\end{align*}
where $\zeta$ denotes the Riemann zeta function.
...
We have shown that a number of proofs of equivalent conditions for (strong) tractability of various types can be unified into a handful of proofs.  Even the arguments underlying these handful of proofs are quite similar and could be consolidated even further.  However some questions remain for further work:
\begin{itemize}
    ...
    \item For Hilbert spaces and solution operators, $\{\SOL_d : \cf_d \to \cg_d\}_{d \in \naturals}$, of tensor product form, how can the equivalent conditions for (strong) tractability be simplified, even when $T(a_1 \cdots a_d,d,\vp) \ne T_1(a_1,d,\vp)\cdots T_1(a_d,d,\vp)$ for some $T_1$?
...
The authors gratefully acknowledge the fruitful discussions with Henryk Wo\'{z}niakowski and Erich Novak.  Fred Hickernell acknowledges support by the United States National Science Foundation under Award No.\ 2316011.
Peter Kritzer was supportedd by the Austrian Science Fund (FWF) Project P34808. For the purpose of open access, the authors have applied a CC BY public copyright licence to any author accepted manuscript version arising from this submission.
...
We give the proof of Proposition \ref{prop:equiv_conditions} here.
