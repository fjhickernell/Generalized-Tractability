...
\begin{equation}
    \Omega  = \{(\varepsilon^{-1},d) : \varepsilon \peter{> E(d) \ge 0}, \ d \in \naturals \},
\end{equation}
%where $E$ is a non-increasing \peter{[{is this a necessary assumption?}]} function.  
...
\[
\begin{cases}
\lambda_{1,d}\ge  \cdots \ge \lambda_{M(d)+1,d} > E(d) \ge\lambda_{M(d)+2,d}, \quad
(\lambda_{1,d}^{-1},d), \ldots, (\lambda^{-1}_{M(d)+1,d},d)  \in \Omega, & E(d) > 0, \\
...
\]
This means that $i(E(d),d,\vp)-1 \ge M(d)+1$, and so $I_d \ge \tI_d$
...
Choose an $\varepsilon \in (E(d),\infty)$ such that $i(\varepsilon,d,\vp) = i(E(d),d,\vp) - 1$.  It follows from \eqref{eq:lambda_K4} that
\[
\lambda_{i(\varepsilon,d,\vp)} \le \varepsilon < E(d) \le \lambda_{i(\varepsilon,d,\vp)} + 1
\]


Note that, for any $\delta>0$,
\[
\lambda_{\lfloor C_{\vp}\, T((E(d) + \delta)^{-1},d,\vp)\rfloor +1} \le E(d) + \delta, 
\]
\peter{Furthermore, define
\[
i (E(d),d,\vp):=\lim_{\delta\to 0}\, i (E(d)+\delta,d,\vp)
= \lim_{\delta\to 0}\, \lfloor C_{\vp}\, T((E(d) + \delta)^{-1},d,\vp)\rfloor +1. 
\]
Note that, for any $\delta>0$,
\[
\lambda_{\lfloor C_{\vp}\, T((E(d) + \delta)^{-1},d,\vp)\rfloor +1} \le E(d) + \delta, 
\]
and therefore, for $\delta\to 0$, we obtain
\[
\lambda_{i (E(d),d,\vp)} \le E(d). 
\]
On the other hand, by definition of $M(d)$, $M(d)$ is minimal such that 
\[
\lambda_{M(d)+2}\le E(d).
\]
This implies that 
\[
M(d) < M(d) +2 \le i (E(d),d,\vp).
\]
...
For $\varepsilon$ taking on all \peter{values in $(E(d),\infty)$}, $i(\varepsilon,d,\vp)$ takes on all values 
\peter{in 
\[
I_{E(d),d}:=\{i(\infty,d,\vp),\ldots, i(E(d),d,\vp)\}
\]
} 
so
\[
i\le C_{\vp}\, T(\varepsilon^{-1},d,\vp)+1 \le C_{\vp} T(\lambda_{i,d}^{-1},d,\vp)+1, \qquad \peter{\forall i\in I_{E(d),d}},  \ d \in \naturals.
\]
...
\[
	...
	[T(\lambda_{i,d}^{-1},d, \vp)]^\tau
	\ge
	\left[\frac{(i-1)}{C_{\vp}}\right]^\tau \qquad  \peter{\forall i\in I_{E(d),d}},  \ d \in \naturals,
 \]
 and this implies
 \[
	 \frac{1}{T (\lambda_{i,d}^{-1},d,\tau \vp)} \le
