...
\begin{align} %\label{eq:comp_equiv}
 ...
 \comp(\varepsilon,d)
	& \le  T(\varepsilon^{-1},d',\vp) \Biggl [\frac{N}{T(\varepsilon^{-1},d',\vp)}  +  \sum_{i=N}^{M(d)} \frac{1}{T(\lambda_{i, d}^{-1},d',\vp)} \Biggr] \quad \forall  d',\ 1 \le N \le M(d). \label{eq:compUP}
\end{align}
...
For $(\varepsilon^{-1}, d) \in \Omega$, define
\[
i(\varepsilon,\vp):= \lfloor C_{\vp}\, T(\varepsilon^{-1},1,\vp)\rfloor +1, \quad
i(\infty,\vp) = \lim_{\varepsilon \to \infty}  i(\varepsilon,\vp) \ge 1 . 
\]
...
\end{enumerate}
Thus, in all cases, the assumption of strong tractability on the restricted domain implies \eqref{eq:strong_rest_tractiff} with $\vp$ replaced by $\vp' = \tau \vp$, and so we see the necessity of $\eqref{eq:strong_rest_tractiff}$.
