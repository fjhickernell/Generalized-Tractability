...
TODO: We should check for consistency of the use of the $\forall$ symbol in displayed formulas. Sometimes we use it, sometimes we do not, sometimes it is preceded by a comma, sometimes not, etc. \fred{Thanks, Kachi}
...
Next we show that if $\vp^* \in \widetilde{\cp}_{\textup{trct}}$, then $\vp^* \in \cp_{\textup{trct}}$.  If \kachi{$\vp^* \in \widetilde{\cp}_{\textup{trct}}$}, then \eqref{eq:tractiff4}  holds for all $\vp \in (\vp^*,\vinfty)$.  By the argument to prove the sufficient condition above, it follows that \eqref{eq:strong_tract_def} must also hold for all $\vp \in (\vp^*,\vinfty)$. Thus, $\vp^* \in \cp_{\textup{trct}}$.
...
\]
\kachi{[KACHI: I have questions regarding $\vp$.]}
...
\end{multline*}
\peter{[Is the latter statement still correct?] \kachi{[I think not.]}}
...
\fred{Fred: Where does $p$ and/or $q$ come in conditions \eqref{eq:KW_exp_spt} and \eqref{eq:KW_exp_pt}?} \kachi{[Kachi: I believe for \eqref{eq:KW_exp_spt} $p$ corresponds to $\tau$ and for \eqref{eq:KW_exp_pt} $p$ corresponds to $\tau_2$ and $q$ corresponds to both $\tau_1$ and $\tau_3$.]}
...
\kachi{In the following example, we provide a brief summary of the above examples \peter{[What does ``these'' refer to?]} of tractability. We would like to emphasize that this example was previously known (see, e.g., \cite{GW06}) where in this case, the roles of $\varepsilon^{-1}$ and $d$ are separated. In order to accomplish this, we will first define two functions $\phi$ and $\psi$ with positive values that are increasing in both of their arguments which will allow to take into account a variety of possible exponents of $\varepsilon^{-1}$ and $d$.
...
\] where in this case $\vp = p$ is a scalar. This can be rewritten as
\[
...
\kachi{The authors gratefully acknowledge the contributions of H. Wo\'{z}niakowski and E. Novak. P. Kritzer acknowledges the support of the Austrian Science Fund, Project ---, which is part of the
Special Research Program “Quasi-Monte Carlo Methods: Theory and Applications”.}
