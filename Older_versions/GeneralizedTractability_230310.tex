...
%\[\comp(\varepsilon,d)\le C_{\vp}\exp\{T(\varepsilon^{-1},d,\vp)\}\quad\quad \forall\varepsilon >0, d\in \mathbb{N}\] 
Since $\lambda_{i,d}$ are non-increasing, we have
\[
\lambda_{\left\lfloor C_{\vp}\exp\{(T(0,d,\vp)(T(\varepsilon^{-1},1,\vp))\}\right\rfloor+1,d}\leq \varepsilon^2
\]
...
\[
j(\varepsilon,d,\vp) = \left\lfloor C_{\vp}\exp\{(T(0,d,\vp)(T(\varepsilon^{-1},1,\vp))\}\right\rfloor+1
\] which attains the values $j = j_d,j_{d+1},\dots$, where
\[
j(\infty,d,\vp)=j_d = \lfloor C_{\vp}\exp\{ T(0,d,\vp)\}\rfloor + 1
\]
Furthermore, we have
\[j \leq C_{\vp}\exp\{(T(0,d,\vp)(T(\varepsilon^{-1},1,\vp))\}+1 \leq C_{\vp}\exp\{(T(0,d,\vp)(T(\lambda^{-1}_{j,d},1,\vp))\}+1 \quad \forall j \geq j_d \]
This implies that for $\tau>1$, we have
\[
\left(\frac{j-1}{C_{\vp}}\right)^{\tau/T(0,d,\vp)} \leq \exp\{T(\lambda^{-1}_{j,d},1,\vp)\}
\]
\[
\exp\{-\tau T(\lambda^{-1}_{j,d},1,\vp\} \leq \left(\frac{C_{\vp}}{j-1}\right)^{\tau/T(0,d,\vp)}\Rightarrow
\exp\{-\tau T(\lambda^{-1}_{j,d},1,\vp)(T(0,d,\vp)\} \leq C_{\vp}^\tau (j-1)^{-\tau}
\]

%\[\frac{1}{T(\lambda_{i,d}^{-1},d,\vp)} \leq \frac{K_{\vp,\tau}}{\left(\log\frac{j-1}{C_{\vp}}\right)^\tau}\]

\textbf{Sufficient condition:} Suppose that (\ref{}) holds for some $\vp \geq 0$. We need to show that for some $C_{\vp}>0$ we have 
\[
\text{comp}(\varepsilon,d) \leq C_{\vp}\exp\{T(0,d,\vp)T(\varepsilon^{-1},1,\vp)\} \quad \forall \varepsilon > , d \in \mathbb{N}
\] Given that $\lambda_{j,d}$ are non-increasing, it follows that $T$ is increasing in its arguments, hence we have the following arguments from previous theorems, i.e.,
\[
\text{comp}(\varepsilon,d) \leq 1 + T(\varepsilon^{-1},d,\vp)\sum_{j=1}^\infty\frac{1}{T(\lambda^{-1}_{j,d},d,\vp)}
\]
Now, we have the following upper bound on the complexity as follows
\begin{align*}
    \comp(\varepsilon,d) &\leq 1+T(\varepsilon^{-1},d,\vp)\sum_{j=1}^\infty\frac{1}{T(\lambda^{-1}_{j,d},d,\vp)}\\
    &\leq 1+T(\varepsilon^{-1},d,\vp)\left[\sum_{j=1}^{C_{\vp}-1}\frac{1}{T(\lambda^{-1}_{j,d},d,\vp)} + \sum_{j=C_{\vp}}^\infty \frac{1}{T(\lambda_{j,d}^{-1},d,\vp)}\right]
\end{align*}
\end{proof}
