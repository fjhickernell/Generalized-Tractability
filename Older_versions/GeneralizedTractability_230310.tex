...
Let $\vp' = \tau \vp$.  Note that $i(\infty,d,\cdot)$ is non-decreasing.  Summing both sides of the inequality over $i$ from $i(\infty,d,\vp') + 1 \ge i(\infty,d, \vp) + 1 \ge 2$ to $\infty$ it follows that
\begin{equation*}
...
\[
\sup_{d\in\naturals} \sigma (c,d,s,\vp) \exp\left(-cd^t\right)<\infty
\]
...
M_{\vp} := \sup_{d\in\mathbb{N}}\sum_{i=1}^\infty \left[\frac{1}{T(\lambda_{i,d}^{-1},d,\vp)}\right]^{T(0,d,\vp)}
<\infty
\end{equation}
...
\[\comp(\varepsilon,d)\le C_{\vp}\exp\{(T(0,d,\vp))(T(\varepsilon^{-1},d,\vp))\}\quad\quad \forall\varepsilon >0, d\in \mathbb{N}\] 
...
\[
\lambda_{\left\lfloor C_{\vp}\exp\{(T(0,d,\vp))(T(\varepsilon^{-1},d,\vp))\}\right\rfloor+1,d}\leq \varepsilon
\]
...
\[
j(\varepsilon,d,\vp) = \left\lfloor C_{\vp}\exp\{(T(0,d,\vp))(T(\varepsilon^{-1},d,\vp))\}\}\right\rfloor+1
\] which attains the values $j = j_d,j_{d+1},\dots$, where
\[
j(\infty,d,\vp)=j_d = \lfloor C_{\vp}\exp\{(T(0,d,\vp))T(0,d,\vp)\}\rfloor + 1
\]
Furthermore, we have
\[j \leq C_{\vp}\exp\{(T(0,d,\vp))(T(\varepsilon^{-1},d,\vp)\}+1 \leq C_{\vp}\exp\{(T(0,d,\vp))(T(\lambda_{j,d}^{-1},d,\vp)\}+1 \quad \forall j \geq j_d \]
This implies that for $\tau>1$, we have
\[
(j-1)^{\tau} \leq C_{\vp}\exp\{\tau(T(0,d,\vp)) (T(\lambda_{j,d}^{-1},d,\vp)\}
\]
This implies
\[
\exp\{-\tau(T(0,d,\vp))( T(\lambda_{j,d}^{-1},d,\vp) \leq \frac{C_{\vp}}{(j-1)^\tau}
\]
We can get
\[
T(\lambda_{i,d},d,\vp))^{-\tau T(0,d,\vp)}\leq C_{\tau,\vp}\frac{1}{(j-1)^\tau} 
\]provided $T(\lambda_{i,d},d,\vp)\geq e$. Therefore, we get
\[
\sum_{i=1}^\infty T(\lambda_{i,d},d,\vp))^{-\tau T(0,d,\vp)} \leq C_{\tau,\vp}\sum_{i=1}^\infty \frac{1}{(j-1)^\tau}
< \infty\]
%\[\frac{1}{T(\lambda_{i,d}^{-1},d,\vp)} \leq \frac{K_{\vp,\tau}}{\left(\log\frac{j-1}{C_{\vp}}\right)^\tau}\]
This yields \eqref{thmqpt}.\\
\textbf{Sufficient condition:} Suppose that (\ref{thmqpt}) holds for some $\vp \geq 0$. We need to show that for some $C_{\vp}>0$ we have 
\[
\text{comp}(\varepsilon,d) \leq C_{\vp}\exp\{(T(0,d,\vp))(T(\varepsilon^{-1},d,\vp)\} \quad \forall \varepsilon > , d \in \mathbb{N}
\] Given that $\lambda_{i,d}$ are non-increasing, it follows that $T$ is increasing in its arguments, hence we have the following arguments from previous theorems, i.e.,
\[
\text{comp}(\varepsilon,d) \leq \left(T(\varepsilon^{-1},d,\vp)\right)^{T(0,d,\vp)}\sum_{j=1}^\infty\left[\frac{1}{T(\lambda^{-1}_{j,d},d,\vp)}\right]^{T(0,d,\vp)}
\]
Now, we have the following upper bound on the complexity as follows
\begin{align*}
    \comp(\varepsilon,d) &\leq T(\varepsilon^{-1},d,\vp)^{T(0,d,\vp)}\sum_{j=1}^\infty\left[\frac{1}{T(\lambda^{-1}_{j,d},d,\vp)}\right]^{T(0,d,\vp)}\\
    &\leq T(\varepsilon^{-1},d,\vp)^{T(0,d,\vp)}\left[\sum_{j=1}^{C_{\vp}-1}\left[\frac{1}{T(\lambda^{-1}_{j,d},d,\vp)}\right]^{T(0,d,\vp)} + \sum_{j=C_{\vp}}^\infty\left[ \frac{1}{T(\lambda_{j,d}^{-1},d,\vp)}\right]^{T(0,d,\vp)}\right]\\
     & \le T(\varepsilon^{-1},d,\vp)^{T(0,d,\vp)}\left[ \left[\frac{C_{\vp}-1}{T(\lambda_{\max}^{-1},d,\vp)}\right]^{T(0,d,\vp)} + M_{\vp}\right]
        \qquad \text{by \eqref{thmqpt}} \\
       & \le T(\varepsilon^{-1},d,\vp)^{T(0,d,\vp)}\left[(C_{\vp})^{T(0,d,\vp)} + M_{\vp}\right]\\ 
       & \le \left(C_{\vp}\,T(\varepsilon^{-1},d,\vp)\right)^{T(0,d,\vp)} + M_{\vp}\, T(\varepsilon^{-1},d,\vp)^{T(0,d,\vp)}\\
       & \le C_{\vp}\,T(\varepsilon^{-1},d,\vp)^{T(0,d,\vp)}
\end{align*}
%It follows that we have $T$-quasi-polynomial tractability.
\end{proof}
\kachi{To do: Find an example for this case.}
