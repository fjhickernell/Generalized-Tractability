...
%Comment these out when we get the J Complexity
...
As defined in \cite{GW11}, we say that a problem is \emph{quasi-polynomially tractable (QPT)} if there exist two constants $C$, $p > 0$, such that
...
\begin{align*}
  ...
  & < \infty.
\end{align*}
Combining this bound with the upper bound on \peter{$S_{2c}$} in \eqref{eq:lastone} establishes the finiteness of $S_{2c}$ and completes the proof of the necessary condition.
...
\begin{equation}
    \Omega  := \{(\varepsilon^{-1},d) : \varepsilon \in (E(d),\infty), \ d \in \naturals \}, \qquad \text{where }E : \naturals \to [0,\infty).
\end{equation}
...
\[
I_{d,\vp}:=\begin{cases} 
\emptyset, & i(E(d),d,\vp) = i(\infty,d,\vp), \\
...
\item If $M(d) <\lceil L_{\vp'} T(0,d,\vp')\rceil$, then 
\[
\sum_{i = \lceil L_{\vp'} T(0,d,\vp')\rceil }^{M(d)} \frac{1}{T (\lambda_{i,d}^{-1},d, \vp')} = 0,
\]
which does not affect the finiteness of $S_{\vp'}$.

\item If $M(d) \ge \lceil L_{\vp'} T(0,d,\vp')\rceil$ and $E(d) = 0$, then $\{\lceil L_{\vp'} T(0,d,\vp')\rceil,  \lceil L_{\vp'} T(0,d,\vp')\rceil + 1, \ldots \} \subseteq I_{d,\vp} \setminus \{1\}$.  

\item If $M(d) \ge \lceil L_{\vp'} T(0,d,\vp')\rceil$ and $E(d) >0$, then \eqref{eq:Mddef} and \eqref{eq:rest_lambda_K4} imply that
\[
\lambda_{i(E(d),d,\vp),d} \le \lambda_{M(d)+1,d} \le E(d) < \lambda_{M(d),d}.
\]
This means that $M(d) \le i(E(d),d,\vp)-1$.  So, $\{ \lceil L_{\vp'} T(0,d,\vp')\rceil, \ldots, M(d)\} \subseteq I_{d,\vp} \setminus \{1\}$. 

In both the second and third cases,
\begin{align*}
S_{\vp'} & = \sup_{d\in\naturals} \sum_{i =  \lceil L_{\vp'} T(0,d,\vp')\rceil}^{M(d)} \frac{1}{T (\lambda_{i,d}^{-1},d, \tau \vp)}
\le 
\sup_{d\in\naturals} \sum_{i \in I_{d,\vp} \setminus\{1\} } \frac{1}{T (\lambda_{i,d}^{-1},1, \tau \vp)} \\
&  \le  K_{\vp,\tau}\, C_{\vp}^\tau
\sum_{i=2}^\infty \frac{1}{(i-1)^\tau}
...
\noindent 
This concludes the proof of Theorem \ref{thm_main_tract2}.
\end{proof}
