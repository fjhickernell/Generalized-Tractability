...
By definition, the \kachi{information} complexity is non-decreasing  as $\varepsilon$ tends to zero.  We also expect it to be non-decreasing as $d$ tends to infinity, since $d$ is typically the number of variables for the functions in $\cf_d$.
...
\kachi{In Table \ref{tab:commonT} below we give common forms of the tractability function, $T$ considered in this paper.}

%\peter{TODO: Maybe add a short paragraph on the notation used in the paper and on its structure? In particular, we should introducean ordering of the parameters $\vp$.}[\kachi{KACHI: I will take care of this}] 
...
A tractability function $T$ is a function of a simple form that provides an upper bound on the \peter{information} complexity of a problem.  Some examples of tractability functions commonly considered are provided in Table \ref{tab:commonT} \peter{[I think we can remove the latter sentence, as it is repeated below.]}.
\kachi{}
\begin{table} 
...
The \peter{concept of} (strong) polynomial tractability for both the algebraic case and the exponential case will be first considered as follows: 
...
be defined by
\[
 T(\varepsilon^{-1},d,\vp)= [\max\{1,\log(1+\varepsilon^{-1})\}]^{\peter{\vp}}, \quad \forall \varepsilon > 0, \,  d \in \naturals, \, \vp \in [0,\infty),
\]
...
\begin{example}[Weak Tractability]
	Suppose that
	\[
	T(\varepsilon^{-1},d,\kachi{(s,t)}) = \max(1,\varepsilon^{-1})^s + d^t, \quad \forall \varepsilon,s,t > 0, \, d \in \naturals, \qquad h(x) = \exp(x) \quad \forall x \ge 0.
	\]
