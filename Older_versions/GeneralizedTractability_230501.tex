...
\[
 \sup_{d\in\naturals}\, d^{-q} \sum_{i=L_{(p,q)} d^{q}}^\infty (\min\{1,\lambda_{i,d}^p\})  < \infty \text{ for some } p,q \ge 0, L_{(p,q)} \in \naturals.
\]
...
\[
 \sup_{d\in\naturals}\, d^{-q} \sum_{i=L_{(p,q)} d^{q}}^\infty [\min\{1,\log(1+\lambda_{i,d}\})]^p < \infty  \text{ for some } p,q \ge 0, L_{(p,q)} \in \naturals.
\]
\end{example}
...
\[
\comp(\varepsilon,d) \leq C_{\vp}\phi(\varepsilon^{-1},p)\psi(1,q) \qquad \forall \varepsilon >0, \, d\in\naturals,
\] where $\vp = (p,q)$. 
...
\end{align*} where $L_{p,\tau_1}>0$ is a constant that may depend on $p$ and $\tau_1$, but is independent of $\varepsilon$, and where $L_{q,\tau_2}>0$ is a constant that may depend on $q$ and $\tau_2$, but is independent of $d$.
\begin{align*}
...
\[
\sup_{d\in\mathbb{N}}\sum_{i=Lp\,\psi(d)} \phi(\lambda_{i,d})\psi(d) < \infty \qquad \text{for some} p \geq 0, L_p \in \naturals.
\]
We have the following special cases. \end{example}
...
\[
\sup_{d\in\mathbb{N}}\frac{1}{d^p}\sum_{i=Lp\,d^{p}} \left(\min\{1,\lambda_{i,d}\} \right)^{p(1+\ln d)} < \infty \qquad \text{for some } p \geq 0, L_p \in \naturals.\]
\end{example}
