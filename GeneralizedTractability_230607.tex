...
\title{\fred{A Unified Treatment of Tractabilty of Approximation Problems}}
...
\end{equation}
The conditions we require $T$ to satisfy are described in  Section \ref{sec:not}, however, the basic idea is that we define our approximation problem to be tractable if 
$\comp(\varepsilon, d) \le  C_{\vp}\, T(\varepsilon^{-1},d,\vp)$ for some constant, $C_{\vp}$, depending only on the parameter $\vp$.  In practice, we expect $T(\varepsilon^{-1},d,\vp)$ to increase sub-exponentially  with increasing $\varepsilon^{-1}$ and/or $d$, however most of our theorems do not require such an assumption.

\Cref{thm_main_strong_tract2,thm_main_tract2,thm_main_rest_strong_tract,thm_main_rest_tract} all take the form

\peter{[PETER: Not meaning to sound too negative, but I am not sure I like the notation with the symbols below. Is it possible to use something different instead?]}
\fred{Fred: I have now given them names above in lines 18-20, so we can experiment with different symbols or letters.}
\begin{multline}
...
A tractability function $T$ is a function of simple form that provides an upper bound on the \peter{information} complexity of a problem.  Some examples of tractability functions commonly considered are provided in Table \ref{tab:commonT}.

\begin{table} 
    \caption{Common forms of the tractability function, $T$. \peter{[PETER: I changed the notation below, using ``algebraic" and ``exponential" first, to be in line with the literature.]}}
{\small
\begin{equation*}
	\begin{array}{r@{\quad}c@{\quad}cc}
		...
		& T(0,d,\vp)\\
		\toprule
		\text{\peter{algebraic strong polynomial}} & \max(1,\varepsilon^{-p}) & 1\\
		\text{\peter{algebraic polynomial}} & \max(1,\varepsilon^{-p})d^{q} & d^{q}\\
		\text{\peter{exponential strong polynomial}} &  [\max(1,\log(1 + \varepsilon^{-1})]^p & 1\\
		\text{\peter{exponential polynomial}} &
		[\max(1,\log(1 + \varepsilon^{-1}))]^p  d^{q} & d^{q} \\
        \text{\peter{algebraic quasi-polynomial}} & 
        \exp\{p(1+\log(\max(1,\varepsilon^{-1}))(1+\log(d))\} &
        \exp\{p(1+\log(d))\}\\
        \text{\peter{exponential quasi-polynomial}} & 
...
\begin{example}[\peter{Algebraic strong} polynomial tractability]

Let the tractability function, $T$,
be defined by
\[
 T(\varepsilon^{-1},d,\vp)= \max\{1,\varepsilon^{-\peter{\vp}}\}
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, \vp \in [0,\infty),
\]  
where in this case \peter{$\vp=p$} is a scalar.
Then Theorem \ref{thm_main_strong_tract2} yields the  equivalent condition for \peter{algebraic strong polynomial tractability}:
\[
...
be defined by
\[
 T(\varepsilon^{-1},d,\vp)= \max\{1,\varepsilon^{-\peter{\vp}}\}\, d^q
...
\begin{example}[\peter{Exponential strong} polynomial tractability]
Let the tractability function, $T$,
be defined by
\[
 T(\varepsilon^{-1},d,\vp)= \left[\max\{1,\log(1+\varepsilon^{-1})\}\right]^{\peter{\vp}}, \quad \forall \varepsilon > 0, \,  d \in \naturals, \, \vp \in [0,\infty),
\]
%\kachi{Alternatively:
%\[
%T(\varepsilon^{-1},d,\vp) = (1+\log\max\{1,\varepsilon^{-1}\})^p
%\]}
where \peter{$\vp=p$} is a scalar. Then Theorem \ref{thm_main_strong_tract2} yields the equivalent condition for \peter{exponential strong polynomial tractability:} 
\[
...
\peter{[PETER: Are we sure that the conditions in Examples 3 and 4 are correct? And, moreover, these conditions should recover the corresponding conditions in \cite{KriWoz19a}. The conditions look different at first sight, but probably it can be shown that they are equivalent... .}

In the following example, we provide a brief summary of these notions of tractability. We would like to emphasize that this example was previously known (see \cite{}). 
...
Furthermore, we assume the following as in \eqref{eq:Tlim}, \eqref{eq:T0}, and \eqref{eq:ptauassume}. We assume that
...
\end{example}
Next, we will introduce another summarized notion of tractability. 
...
We say that a problem is non-separable tractable with parameter $p$ iff there exists a positive $C_{p}$ such that

\[\comp(\varepsilon,d) \leq  C_{p}\exp\{p(\phi(\varepsilon^{-1})\psi(d))\}.\]
...
For the case $\theUB(d)> 0$, we utilize upper bound \eqref{eq:compUP_rest}, specializing to the case of $\thed=d$ and $N = \min\left(\lceil L_{\vp}\, T(0,d,\vp)\rceil,\theUB(d)\right)$:
\begin{align*}
       \comp(\varepsilon,d)
