...
\begin{align} %\label{eq:comp_equiv}
	...
	\intertext{Note that in this last expression, $n \ge N$ is enforced since $n$ is an integer strictly greater than  $N-1$ by definition.  This then implies that}
	\comp(\varepsilon, d) 
...
\end{example}
\kachi{
\begin{example}[Separable tractability]
Let the tractability function, $T$,
be defined by
\[
 ...
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p,q \ge 0.
\]
with non-decreasing functions $\phi : (0,\infty) \times (0,\infty) \to (0,\infty)$ and $\psi : \naturals  \times (0,\infty) \to (0,\infty)$. In addition, satisfying the following conditions as in (\ref{eq:ptauassume}) 
\begin{align*}
 (\phi(\varepsilon^{-1},p))^{\tau_1} &\le K_{p,\tau_1} \phi (\varepsilon^{-1},\tau_1 p)\\
  (\psi(d,q))^{\tau_2} &\le K_{q,\tau_2} \psi (d,\tau_2 q)\\
\end{align*}
where $\tau_1, \tau_2>0$. %As the name implies, the roles of $\varepsilon^{-1}$ and $d$ are separated. 
\end{example}
...
\[
 ...
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p,q \ge 0.
\]  with non-decreasing functions $f_1$, $f_2 : [1,\infty)\rightarrow[1,\infty)$.
\end{example}
}
