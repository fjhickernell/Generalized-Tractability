...
be defined by
\[
 T(\varepsilon^{-1},d,p)= \exp\{p(1+\log(\max\{1,\varepsilon^{-1}\})(1+\log d)\}
\]
Observe that this is a special case of the $T$-tractability function, where the function is polynomial of exponent $1+\log d$ and $1+\log\max\{\varepsilon^{-1},1\}$.
...
be defined by
\[
 T(\varepsilon^{-1},d,(p,q))= \phi(\varepsilon^{-1},p)\, \psi(d,q)
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p,q \ge 0.
\]
with non-decreasing functions $\phi : (0,\infty) \times (0,\infty) \to (0,\infty)$ and $\psi : \naturals  \times (0,\infty) \to (0,\infty)$. In addition, we assume the following conditions as in (\ref{eq:ptauassume}) 
%\begin{align*}
 %(\phi(\varepsilon^{-1},p))^{\tau_1} &\le K_{p,\tau_1} \phi (\varepsilon^{-1},\tau_1 p)\\ (\psi(d,q))^{\tau_2} &\le K_{q,\tau_2} \psi (d,\tau_2 q)\\\end{align*}
 \begin{align*}
     [T(\varepsilon^{-1},d,(p,q))]^\tau \leq K_{(p,q),\tau}T(\varepsilon^{-1},d,(\tau p,\tau q))
 \end{align*}
where $\tau>0$.
Observe that for this $T$, we have strong tractability if the following condition is satisfied
\[
 T(\varepsilon^{-1},1,p)= \phi(\varepsilon^{-1},p)\, \psi(1,0) = \phi(\varepsilon^{-1},p)
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p \ge 0.
\]
Thus, Theorem \ref{thm_main_strong_tract2} yields the following equivalent. We say that a problem is strongly separable tractable iff there exists $p>0$ and an integer $C(p)\ge 0$ such that
\begin{equation} \label{eq:strong_tractiff}
     \sup_{d \in \naturals} \sum_{i = L(p,1)}^\infty \frac{1}{\phi(\lambda_{i, d}^{-1},p)} < \infty.
\end{equation}
...
\[
 ...
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p\ge 0.
\]  with non-decreasing functions $\phi : (0,\infty) \times (0,\infty) \to (0,\infty)$ and $\psi : \naturals  \times (0,\infty) \to (0,\infty)$.
\end{example}
}
...
\begin{theorem}\label{thm_main_strong_tract3}
Let $T$ be a tractability function as specified in \eqref{eq:Tspec} and satisfying \eqref{eq:Tconditions} and \eqref{eq:ptauassume}.  A problem is strongly $T$-tractable iff there exists $\vp \in [\vzero, \boldsymbol{\infty})$ and an integer $L_{\vp} > 0$ such that
\begin{equation} \label{eq:strong_tractiff3}
