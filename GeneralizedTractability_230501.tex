...
where $\vp$ is a scalar. Then Theorem \ref{thm_main_strong_tract2} yields the equivalent condition for strong exponential polynomial tractability \[
 \sup_{d\in\naturals} \sum_{i=L_{p} }^\infty (\min\{1,\log(1+\lambda_{i,d}\})^{p}  < \infty.
...
Observe that for this tractability function $T$, we have strong tractability if the following condition is satisfied
\[
 T(\varepsilon^{-1},1,\vp)= \phi(\varepsilon^{-1},p)\, \psi(1,0) = \phi(\varepsilon^{-1},p)
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p \ge 0,
\] where in this case $\vp$ is a scalar. Furthermore, we assume the following technical conditions. There exists a $K_{(p,q),\tau}$ depending on $p,q$ and $\tau$, but independent of $\varepsilon$ and $d$, such that
\begin{align*}
...
\end{example}
\begin{example}[Non-separable symmetric tractability]
    Let the tractability function $T$, be defined by
\[
 T(\varepsilon^{-1},d,(p,q))= \exp\{\phi(\varepsilon^{-1},p)\, \psi(d,q)\}
 \qquad \forall \varepsilon > 0, \,  d \in \naturals, \, p\ge 0.
\]  with non-decreasing functions $\phi : (0,\infty) \times (0,\infty) \to (0,\infty)$ and $\psi : \naturals  \times (0,\infty) \to (0,\infty)$. Furthermore, we assume the following technical conditions. There exists a $K_{(p,q),\tau}$ depending on $p,q$ and $\tau$, but independent of $\varepsilon$ and $d$, such that
\begin{align*}
     [\phi(\varepsilon^{-1},p)]^{\tau} &\le K_{(p,1),\tau} \phi (\varepsilon^{-1},\tau p)\\
      [\psi(d,q)]^{\tau} &\le K_{(1,q),\tau} \psi (d,\tau q) \qquad \forall \, \varepsilon \in (0,\infty),\, d\in \naturals,\, p,q\geq 0,\, \tau >0.
\end{align*}
...
(\lambda_{1,d}^{-1},d), \cdots, (\lambda^{-1}_{M(d)+1,d},d) \} \in \Omega.
\]
