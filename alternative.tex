...
\[
 \sup_{d\in\naturals} \sum_{i=L_{(p,q)} d^{q}}^\infty [\min\{1,\log(1+\lambda_{i,d}\})]^p d^{-q} < \infty  \text{ for some } p,q \ge 0, L_{(p,q)} \in \naturals.
\]
\end{example}


\begin{example}[Algebraic quasi-polynomial tractability] Here we consider another common notions of tractability. 
...
\[
\sup_{d\in\mathbb{N}}\sum_{i=Lp\,e^pd^{p}} \left(\min\{1,\lambda_{i,d}\} \right)^{p(1+\ln d)}e^{-p}d^{-p} < \infty \qquad \text{for some} p \geq 0, L_p \in \naturals.
\]
\end{example}
...
\[
\sup_{d\in\mathbb{N}}\sum_{i=Lp\,e^pd^{p}} \left(\min\{1,\log(1+\lambda_{i,d})\} \right)^{p(1+\ln d)}e^{-p}d^{-p} < \infty
\]
\end{example}
...
\[
\comp(\varepsilon,d) \leq  C_{\vp}\,T(\varepsilon^{-1},d,\vp)\qquad \forall \varepsilon >0, \, d\in\naturals,
\]
that is
\[
\comp(\varepsilon,d) \leq \phi(\varepsilon^{-1},p)\, \psi(d,q) \qquad \forall \varepsilon >0, \, d\in\naturals.
\]
...
\[
\comp(\varepsilon,d) \leq C_{\vp}T(\varepsilon^{-1}, 1,\vp) \qquad \forall \varepsilon >0, \, d\in\naturals,
\] where $\vp = p$. That is, we have
\[
\comp(\varepsilon,d) \leq \phi(\varepsilon^{-1},p)\qquad \forall \varepsilon >0, \, d\in\naturals.
\]
Furthermore, we assume the following as in \eqref{eq:Tconditions} and \eqref{eq:ptauassume}.
\[
\lim_{\varepsilon\rightarrow0}T(\varepsilon^{-1},d,\vp) = \infty \qquad \forall d\in\naturals, \vp \in[0,\infty)^2.
\]
Also since $T(\cdot,d,\vp)$ is non-decreasing, we define the following limit
\[
T(0,d,\vp):= \lim_{\varepsilon \rightarrow \infty}T(\varepsilon^{-1},d,\vp)\geq T(0,1,\textbf{0})>0
\] 
{\color{red} Additionally, we have the following technical assumptions. Suppose that, for any real $\tau>0$ we have
\begin{align*}
  [T(\varepsilon^{-1},d,\vp)]^\tau \leq K_{\vp,\tau}T(\varepsilon^{-1},d,\tau\vp) \quad \forall \varepsilon \in (0,\infty),\, d\in \mathbb{N},\, \vp\in[0,\infty)^2, \tau >0,\end{align*} that is for positive $K_{\vp,\tau}$ we have by definition
  \[
  [\phi(\varepsilon^{-1},p)\, \psi(d,q) ]^\tau \leq K_{\vp,\tau} \phi(\varepsilon^{-1},\tau p)\, \psi(d,\tau q) \]
%Thus, Theorem \ref{thm_main_strong_tract2} yields the following equivalent condition for strongly separable tractability
%\begin{equation*} \label{eq:strong_tractiff}    \sup_{d \in \naturals} \sum_{i = L_p}^\infty \frac{1}{\phi(\lambda_{i, d}^{-1},p)} < \infty.\end{equation*}
Theorem \ref{thm_main_tract2} yields the following equivalent condition for 
separable tractability
\[
 \sup_{d \in \naturals} \frac{1}{\psi(d,q)} \sum_{i = L_{\vp\psi(d,q)}}^\infty \frac{1}{\phi(\lambda_{i, d}^{-1},p)} < \infty.
\]
Observe that the only place where assumption (\ref{eq:ptauassume}) is used is in the necessary condition as follows.
Suppose that we have
$T$-separable tractability. That is, for some $\vp \ge \vzero$, there exists a positive constant $C_{\vp}$ such that
\[
\comp(\varepsilon,d)\le C_{\vp}\, T(\varepsilon^{-1},d,\vp) \qquad \forall \varepsilon > 0,  \ d \in \naturals.
\]
Since the sequence of eigenvalues $\lambda_{1,d}, \lambda_{2,d}, \ldots $ is non-increasing, we have
...
\] That is
\[
i \leq C_{\vp}\phi(\varepsilon^{-1},p)\psi(d,q)+1\leq C_{\vp}\phi(\lambda^{-1}_{i,d},p)\psi(d,q)+1
\]
This implies via our technical assumption \eqref{eq:ptauassume} that for any $\tau > 1$,
\begin{gather*}
	K_{\vp,\tau}\,\phi (\lambda_{i,d}^{-1},\tau p)\psi(d,\tau q) \ge
	[\phi(\lambda_{i,d}^{-1}, p)\psi(d,q)]^\tau
	\ge
	\left[\frac{(i-1)}{C_{\vp}}\right]^\tau \qquad \forall i \ge i(\infty,d,\vp),  \ d \in \naturals, \\
	 \frac{1}{\phi (\lambda_{i,d}^{-1},\tau p)\psi(d,\tau q) } \le
	\frac{K_{\vp,\tau}\, C_{\vp}^\tau}{(i-1)^\tau}\qquad \forall i \ge \max\{i(\infty,\vp),2\},  \ d \in \naturals.
\end{gather*}

Let $\vp' = (\tau p,\tau q)$.  Note that $i(\infty,d,\cdot)$ is non-decreasing.  Summing both sides of the latter inequality over $i$ from $i(\infty,d,\vp') + 1 \ge i(\infty,d, \vp) + 1 \ge 2$ to $\infty$ it follows that
\begin{equation*}
	\sup_{d\in\naturals} \, \, \sum_{i=i (\infty,d,\vp')+1}^\infty \frac{1}{T (\lambda_{i,d}^{-1},d,\vp')}
	 \le  K_{\vp,\tau}\, C_{\vp}^\tau
	\sum_{i=2}^\infty \frac{1}{(i-1)^\tau} \\
...
%As the name implies, the roles of $\varepsilon^{-1}$ and $d$ are separated. 
}
\end{example}
\begin{example}[Non-separable tractability]
    Let the tractability function $T$, be defined by
\[
...
\[
\comp(\varepsilon,d) \leq C_{\vp} T(\varepsilon^{-1},d,\vp),
\]
for all $\varepsilon\in (0,1]$ and $d\in \naturals$, that is
\[\comp(\varepsilon,d) \leq  \exp\{p(\phi(\varepsilon^{-1})\psi(d))\}.\]
...
\sup_{d\in\mathbb{N}}\sum_{i=Lp\,e^pd^{p}} \left(\min\{1,\lambda_{i,d}\} \right)^{p(1+\ln d)}e^{-p}d^{-p} < \infty \qquad \text{for some} p \geq 0, L_p \in \naturals.
\]
We observe that Example 5 is a special case  where in this case the function $\phi(\varepsilon^{-1})= 1+\log(\max\{1,\varepsilon^{-1}\})$ and $\psi(d) = 1+\log(d)$.
\end{example}
...
Suppose that the domain of interest $(\varepsilon^{-1},d)$ is not all of $(0,\infty) \times \naturals$ but given by 
\begin{equation}
    \Omega  = \{(\varepsilon^{-1},d) : \varepsilon \peter{\ge E(d) > 0}, \ d \in \naturals \},
\end{equation}
where $E$ is a non-increasing \peter{[{is this a necessary assumption?}]} function.  A problem is \peter{strongly}
...
Define $M(d) = \max\{n : \lambda_{n+1,d} \ge E(d)\}$, i.e., 
\[
\lambda_{1,d}\ge  \cdots \ge \lambda_{M(d)+1,d} \ge E(d) > \lambda_{M(d)+2,d}, \qquad
(\lambda_{1,d}^{-1},d), \ldots, (\lambda^{-1}_{M(d)+1,d},d)  \in \Omega.
\]
...
\begin{proof}
    \textbf{Sufficient Condition:}\\
We adapt the argument in the proof of the sufficient condition for Theorems \ref{thm_main_strong_tract2} and \ref{thm_main_tract2}.  Suppose that \eqref{eq:rest_tractiff4} holds for some $\vp \ge 0$. Fix any $(\varepsilon^{-1},d) \in \Omega$.  Then 

Since the $\lambda_{i,d}$ are non-increasing in $i$, it follows that the $T(\lambda_{i,d}^{-1},d,\vp)$ are non-decreasing in $i$ for $i \in \{1, \ldots, M(d)+1\}$. In particular, for any \peter{$N\in\{1,\ldots,M(d)\}$ and any} $n \in \{N, \ldots, M(d)\}$
\begin{align*}
    \lambda_{n+1,d} \le \lambda_{n,d} \le \cdots \le \lambda_{N,d}
    & \implies \frac{1}{T(\lambda_{n+1, d}^{-1},d,\vp)} \le \frac{1}{T(\lambda_{n, d}^{-1},d,\vp)} \le \cdots \le \frac{1}{T(\lambda_{N, d}^{-1},d,\vp)} \\
     \implies \frac{1}{T(\lambda_{n+1, d}^{-1},d,\vp) }
...
\end{equation}
Define the integers \fred{Only got here because we have to be careful with the domain of stuff.} 
\peter{[PETER: Adding something we might use in color, but not deleting any of the things we already have.]}
\[
i (\varepsilon,d,\vp):= \lfloor C_{\vp}\, T(\varepsilon^{-1},d,\vp)\rfloor +1, \qquad
i (\infty,d,\vp) := \lfloor C_{\vp}\, T(0,d,\vp)\rfloor +1 \ge 1.
\]
...
since
$T(\cdot,d,\vp)$ is non-decreasing
\peter{{[Actually, the }same should hold $\forall (\varepsilon^{-1},d)\in\Omega$, right? ]} .



For $\varepsilon$ taking on all positive values \peter{(values in $[E(d),\infty)$)}, $i(\varepsilon,d,\vp)$ takes on all values greater than or equal to $i(\infty,d,\vp)$
\peter{(in $\{i(\infty,d,\vp),\ldots, i(E(d),d,\vp)\}$)}, so
\[
i\le C_{\vp}\, T(\varepsilon^{-1},d,\vp)+1 \le C_{\vp} T(\lambda_{i,d}^{-1},d,\vp)+1, \qquad \forall i \ge i(\infty,d,\vp),  \ d \in \naturals.
\]
\peter{[Could we not go through the same argument as below with $i$ being in the set $\{i(\infty,d,\vp),\ldots, i(E(d),d,\vp)\}$?]}
