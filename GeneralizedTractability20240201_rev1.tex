...
\comp:(0,\infty) \times \naturals \to \natzero, 
\]
where $\comp(\varepsilon,d)$ is the number of linear functionals required by the best admissible algorithm to satisfy the error criterion \eqref{eq:errcrit}\fred{, and  $\naturals_0$ denotes the set of non-negative integers}.
...
\peter{Strictly speaking, we are considering the \textit{absolute} error criterion in this paper.
...
\peter{We want to bound the information complexity in terms of a simple function, $T$, of $\varepsilon^{-1}$, \kachi{$d$, and some parameter $\vp$}, and then identify conditions on the singular values that are equivalent to the fact that this bound holds, and are easier to verify than showing the bound directly. By ``simple'' we mean that $T$ takes a form such that the conditions in the theorems of this paper can be checked effectively; however, we remark that this is also related to the singular values $\lambda_{i,d}$ of the solution operator. Depending on those, a function $T$ may be useful for one particular problem setting but not for another.} \kachi{In Table \ref{tab:sampleT}, we present various common notions of tractability studied in the literature, characterized by $T$.} 
\begin{table}[hbt!]
    \caption{\kachi{Common notions of tractability}}
{\small
\begin{equation*}
...
\end{equation}
The conditions we require $T$ to satisfy are described in  Section \ref{sec:not}, however, the basic idea is that we define our approximation problem to be tractable if $\comp(\varepsilon, d) \le  C_{\vp}\, T(\varepsilon^{-1},d,\vp)$ for some constant\footnote{\peter{By ``constant'' we mean that the term is independent of the \emph{variables} $\varepsilon$ and $d$, but may depend on the \emph{parameter} $\vp$.}}, $C_{\vp}$, depending only on the parameter $\vp$. \kachi{For instance, when considering (algebraic) polynomial tractability, the parameter $\vp$ represents the exponents $(p,q)$ of $\varepsilon$ and $d$, as illustrated in Table 1}.  In practice, we expect $T(\varepsilon^{-1},d,\vp)$ to increase sub-exponentially with increasing $\varepsilon^{-1}$ and/or $d$, however, most of our theorems do not require such an assumption.

The parameter $\vp$ is an $s$-dimensional vector with $s \geq 1$. \fred{We write $\vzero$ to denote a vector with all components equal to zero, and $\vinfty$ to denote a vector with all components equal to $\infty$.
...
\peter{where $T(0,\thed,\vp)$ is defined as in \eqref{eq:T0} below.}  \fred{These theorems say essentially that \emph{(strong) tractability is equivalent to summability conditions on the singular values.} A more stringent form of tractability resulting from a smaller $T$ is equivalent to a more stringent summability condition since the terms being summed become larger.}

Although some of \Cref{thm_main_strong_tract2,thm_main_tract2,thm_main_rest_strong_tract,thm_main_rest_tract} are special cases of others, we prove each separately to allow the reader to become familiar with the arguments used as additional layers of complexity are added. \peter{(This may be, e.g., additional
...
\end{equation}
\peter{We remark that the latter assumption is quite natural, and is, for example fullfilled for all tractability types in Table~\ref{tab:commonT}.}
...
\end{align*}
\peter{where the infinite sum is guaranteed to exist by the condition \eqref{eq:strong_tractiff3}.}
...
The proof of this result is deferred to \peter{Appendix 2} where we will prove this statement for the latter two conditions, i.e., for the conditions on exponential polynomial tractability. The case of exponential strong polynomial tractability is not explicitly included, as it is actually simpler and can be treated analogously.  \peter{We give a short example of how we can apply the condition \eqref{eq:exp_spt} to a concrete problem in Appendix 1.} 
...
\fred{Help!  I do not understand how we give and example of a summary of examples above.}
...
\]
This implies via our technical assumption \eqref{eq:ptauassume} that, \peter{for $\tau>1$,}
\begin{gather}
\nonumber
 K_{\vp,\tau}\,T (\lambda_{i,d}^{-1},1,\tau \vp) \ge
 [T(\lambda_{i,d}^{-1},1, \vp)]^\tau
...
\begin{align*}
       \comp(\varepsilon,d)
       & \le T(\varepsilon^{-1},d, \vp) \Biggl [ \frac{\min\left\{\lceil L_{\vp}\, T(0,d,\vp)\rceil,\theUB(d)\right\}}{T(\varepsilon^{-1},d, \vp)}  + \underbrace{\sum_{i = \min\left\{\lceil L_{\vp}\, T(0,d,\vp)\rceil,\theUB(d)\right\}}^{\theUB(d)} \frac{1}{T(\lambda_{i, d}^{-1},d,\vp)}}_{\le S_{\vp} + 1/ T(\lambda_{\theUB(d), d}^{-1},d,\vp) \text{ by \kachi{\eqref{eq:rest_tractiff}}} }
        \Biggr] \\
...
\section*{\peter{Appendix 1: A concrete example}}
...
$\bsb=\{b_j\}_{j \ge 1}$ be two sequences of real positive weights
such that
\begin{equation}\label{eq:cond_a_b}
b_\ast:=\inf_j b_j> 0\ \ \ \ \ \mbox{and}\ \ \ \ \
a_\ast:=\inf_j a_j>0.
\end{equation}
...
We define $\ch_{\kor,d,\bsa,\bsb}$ as the space of all one-periodic functions $f$ with absolutely convergent Fourier series, 
...
\]
Let us consider $L_2$-approximation, i.e., $\SOL_d:\ch_{\kor,d,\bsa,\bsb}\to L_2([0,1]^d)$, $\SOL_d(f)=f$, and let us assume
$\Lambda^{\rm all}$ as the information class. One can show that the singular values  for this choice of $\SOL_d$ are given by
\begin{equation}\label{eq:eigenvalues}
...
\[
 \sum_{i=1}^\infty \frac{1}{T\left(\lambda_{i,d}^{-1},1,(p,0)\right)}\kachi{,}
\]
...
\[
  ...
  = \sum_{\bsh\in\ZZ^d} \frac{1}{\left(1+ \log \left(\omega^{-\frac{1}{2}\sum_{j=1}^d a_j \abs{h_j}^{b_j}}\right)\right)^p}\, .
\]
It is then not hard to show that the two conditions 
\[
 \sum_{j=1}^\infty \frac{1}{b_j} <\infty \quad \mbox{and}\quad \alpha^*:=\liminf_{j\to\infty}\frac{\log a_j}{j}>0
\]
...
\section*{\peter{Appendix 2: Proof of Proposition~\ref{prop:equiv_conditions}}}
