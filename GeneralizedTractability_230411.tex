...
a whole sequence of problems  $\SOL_d : \cf_d \to \cg_d$, for $d\in\naturals$, and we furthermore assume} that the Hilbert spaces are defined in such a way that $\lambda_{j,\cdot}$ is non-decreasing, although this assumption is not needed for our results.
...
Note that the tractability function is bounded below away from zero even as $\varepsilon \to \infty$ \peter{[PETER: Is it?]}, so that the information complexity is not expected to vanish under arbitrarily loose error requirements.  For this definition of tractability to make sense we assume that
