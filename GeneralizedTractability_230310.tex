...
\begin{align*}
       \comp(\varepsilon,d)
       & = 1 + T(\varepsilon^{-1},1, \vp) \Biggl [ \sum_{i=1}^{L(\vp)-1} \frac{1}{T(\lambda_{i, d}^{-1},1,\vp)}
...
This yields \eqref{eq:strong_tractiff3} with $\vp$ replaced by $\vp' = \tau \vp$ and $L(\vp') = j (\infty,\vp)$, and so we see the necessity of $\eqref{eq:strong_tractiff3}$.
