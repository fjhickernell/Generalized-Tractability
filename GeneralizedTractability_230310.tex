...
If \eqref{eq:tract_def} holds for some $\vp$, it clearly holds for larger, $\vp$.  We are often interested in the optimal or smallest $\vp$ for which \eqref{eq:tract_def} holds.  Let
\begin{equation*}
...
If $\vp^* \in \cp_{\textup{strct}}$, then \eqref{eq:strong_tract_def} holds for all $\vp \in (\vp^*,\vinfty)$.  By the argument to prove the necessary condition, it follows that \eqref{eq:strong_tractiff3} must also hold for all $\vp \in (\vp^*,\vinfty)$, so $\vp^* \in \widetilde{\cp}_{\textup{strct}}$.  Conversely, if $\vp^* \in \widetilde{\cp}_{\textup{strct}}$, then \eqref{eq:strong_tractiff3}  holds for all $\vp \in (\vp^*,\vinfty)$.  By the argument to prove the sufficient condition, it follows that \eqref{eq:strong_tract_def} must also hold for all $\vp \in (\vp^*,\vinfty)$, so $\vp^* \in \cp}_{\textup{strct}}$.
...
Let
\[
j=j (\varepsilon):= \lfloor C_{\vp}\, T(\varepsilon^{-1},d,\vp)\rfloor +1.
\]
If we vary $\varepsilon\in (0,\infty)$, we see that $j=j_d^*, j_d^*+1, j_d^*+2,\ldots$, where
\[
  j_d^*=\lfloor C_{\vp}\, T(0,d,\vp)\rfloor +1
\]
Note that
\[
j\le C_{\vp}\, T(\varepsilon^{-1},d,\vp) +1.
\]
Due to our assumptions, we know that
$T(\cdot,d,\vp)$ is increasing in the first argument if
we keep $p$ and $q$ fixed.

Then, by \eqref{eq:lambda_K4},
\[
j \le C_{\vp}\, T(\varepsilon^{-1},d,\vp) +1 \le C_{\vp}\, T(\lambda_{j,d}^{-1},d,\vp) +1,
\]
and the latter chain of inequalities holds for all $j\ge j_d^*$.

This implies
\[
T(\lambda_{j,d}^{-1},d, \vp) \ge \frac{j-1}{C_{\vp}}.
\]
This, in turn, implies, for any $\tau>1$, that
\[
 (T(\lambda_{j,d}^{-1},d,\vp))^\tau \ge \frac{(j-1)^\tau}{C_{\vp}^\tau},
\]
which yields
\[
 K_{\vp,\tau}\, T (\lambda_{j,d}^{-1},d,\tau p,\tau q) \ge (T(\lambda_{j,d}^{-1},d, \vp))^\tau
 \ge \frac{(j-1)^\tau}{C_{\vp}^\tau}.
\]
This then implies
\[
\sum_{j=j_d^*}^\infty \frac{1}{T(\lambda_{j,d}^{-1},d, \tau p,\tau q)}
\le K_{\vp,\tau}\, C_{\vp}^\tau \sum_{j=j_d^*}^\infty \frac{1}{(j-1)^\tau}
\peter{\le K_{\vp,\tau}\, C_{\vp}^\tau \, \zeta(\tau)}
<\infty.
\]
\peter{
Consequently,
\[
\sup_{d\in\naturals} \sum_{j=j_d^*}^\infty \frac{1}{T(\lambda_{j,d}^{-1},d, \tau p,\tau q)} \le K_{\vp,\tau}\, C_{\vp}^\tau \, \zeta(\tau) < \infty.
\]
}
Note now that, \peter{for any $d\in\naturals$,}
\[
 j_d^* = \lfloor C_{\vp}\, T(0,d,\vp)\rfloor +1 \le \lfloor C_{\vp}^\tau\, T(0,d,\tau p,\tau q)\rfloor +1.
\]
