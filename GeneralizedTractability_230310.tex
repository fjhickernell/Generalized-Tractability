...
\end{equation}
The question is,``How hard is this problem as $\varepsilon$ tends to zero or as $d$ tends to infinity?''  We want to identify equivalent conditions in terms of the singular values that are easier to verify than dealing with the definition in \eqref{eq:comp_def}.
...
\]
A tractability function is a function of simple form that provides an upper bound on the computational complexity of a problem.  Some examples of tractability functions considered previously are provided in Section \ref{sec:examples}.
...
\end{equation*}
for any $d' \in \naturals$.  Combining this expression for the complexity with the implication in \eqref{eq:implicate_a}, we can conclude from the previous line that
...
\begin{align*}
       ...
       & \le 1+ T(\varepsilon^{-1},1,\vp) \underbrace{\left[\frac{(L(p)-1)}{T(\lambda_{\max}^{-1},1,\vp)} + S_{\vp}\right]}_{=:C_{1,\vp}}\\
       & = 1+ T(\varepsilon^{-1},1,\vp) C_{1,\vp}\\
...
\end{equation*}
Summing both sides of the inequality yields it follows that
\begin{equation*}
\sup_{d\in\naturals} \sum_{j=j (\infty,\vp)}^\infty \frac{1}{T (\lambda_{j,d}^{-1},1, \tau \vp)}
\le K_{\vp,\tau}\, C_{\vp}^\tau
\sum_{j=j (\infty,\vp)}^\infty \frac{1}{(j-1)^\tau} \le
