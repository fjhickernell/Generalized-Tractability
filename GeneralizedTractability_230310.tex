...
For a fixed $d$, define $\varepsilon^{-1}_{\max,d} = \max(V_c - d,\lambda_{\max}^{-1})$.  the largest the largest $\varepsilon$ can be and still satisfy $ \varepsilon^{-1} + d \ge V_c$ is $\max(V_c - d,1)$.   Let 
\begin{equation} \label{eq:nstardef}
	\varepsilon^{-1}_{\max,d} := \max(V_c - d,\lambda_{\max}^{-1}), \qquad n^*(d,c) : = n(\varepsilon^{-1}_{\max,d},d,c) \le h(cT(\varepsilon^{-1}_{\max,d},d)).
\end{equation} 
By varying $\varepsilon^{-1}$ in the interval $[\varepsilon^{-1}_{\max,d},\infty)$,  the integer $n(\varepsilon,d,c)$ attains all values greater than or equal to $n^*(d,c)$.  Thus, 
\begin{align}
...
\right] 
 ...
 \label{eq:lastone}
		 & \le \sup_{d\in\naturals} \left [
		 \frac{ h(cT(\varepsilon^{-1}_{\max,d},d))}{h(2cT(\lambda_{\max}^{-1},d))}
		\right] + \frac{\pi^2}{6}
...
Note that 
\[
\varepsilon^{-1}_{\max,d} = \max(V_c - d,\lambda_{\max}^{-1}) 
= \begin{cases} V_c - d \le V_c, & d \in \{1, \ldots, d^* : = \lfloor V_c - \lambda_{\max}^{-1} \rfloor\} \\
\lambda_{\max}^{-1}, & d \in \{d^*+1, \ldots\}
\end{cases}
\]
...
\begin{align*}
  ...
  \\
  & \le \max \left \{  
		 \frac{ h(cT(V_c,d^*))}{h(2cT(\lambda_{\max}^{-1},1))}, 
  \sup_{d\in \{d^*+1, \ldots \}}  \,
  ...
  & = \max \left \{  
		 \frac{ h(cT(V_c,d^*))}{h(2cT(\lambda_{\max}^{-1},1))}, 
  \sup_{d\in \{d^*+1, \ldots \}} \,
		 \frac{1}{h(cT(\lambda_{\max}^{-1},d))}
  \right \} \\
  & < \infty \qquad \text{since }h \ge 1.
\end{align*}
Combining this bound with \eqref{eq:lastone} establishes the finiteness of $S_2c$ and completes the proof of the necessary condition.
